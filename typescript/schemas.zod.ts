...
export const Config = z.object({ "RPID": z.string().describe("A valid domain that identifies the Relying Party. A credential can only by used  with the same enity (as identified by the `RPID`) it was registered with."), "RPDisplayName": z.string().describe("Friendly name for the Relying Party (application). The browser may display this to the user."), "RPOrigins": z.array(z.string()).describe("Configures the list of Relying Party Server Origins that are permitted. These should be fully qualified origins."), "AttestationPreference": z.any().superRefine((x, ctx) => {
    const schemas = [z.literal("#/$defs/PreferNoAttestation"), z.literal("#/$defs/PreferIndirectAttestation"), z.literal("#/$defs/PreferDirectAttestation"), z.literal("#/$defs/PreferEnterpriseAttestation")];
    const errors = schemas.reduce(
      (errors: z.ZodError[], schema) =>
        ((result) => ("error" in result ? [...errors, result.error] : errors))(
          schema.safeParse(x)
  ...
  }).describe("Preferred attestation conveyance during credential generation").default("none"), "AuthenticatorSelection": z.literal("#/$defs/AuthenticatorSelection").describe("Login requirements for authenticator attributes."), "Debug": z.boolean().describe("Enables various debug options."), "EncodeUserIDAsString": z.boolean().describe("Ensures the user.id value during registrations is encoded as a raw UTF8 string. This is useful when you only use printable ASCII characters for the random user.id but the browser library does not decode the URL Safe Base64 data."), "Timeouts": z.literal("#/$defs/TimeoutsConfig").describe("Configures various timeouts."), "RPIcon": z.string().describe("@deprecated This option has been removed from newer specifications due to security considerations."), "RPOrigin": z.string().describe("@deprecated Use RPOrigins instead."), "Timeout": z.number().int().describe("@deprecated Use Timeouts instead.") }).strict().describe("Configuration and default values for the {@link WebAuthn4JS} instance.");
export const Credential = z.object({ "ID": z.string().describe("A probabilistically-unique byte sequence identifying a public key credential source and its authentication assertions."), "PublicKey": z.string().describe("The public key portion of a Relying Party-specific credential key pair, generated by an authenticator and returned to a Relying Party at registration time."), "AttestationType": z.string().describe("The attestation format used (if any) by the authenticator when creating the credential."), "Transport": z.array(z.any().superRefine((x, ctx) => {
    const schemas = [z.literal("#/$defs/TransportUSB"), z.literal("#/$defs/TransportNFC"), z.literal("#/$defs/TransportBLE"), z.literal("#/$defs/TransportHybrid"), z.literal("#/$defs/TransportInternal")];
    const errors = schemas.reduce(
      (errors: z.ZodError[], schema) =>
        ((result) => ("error" in result ? [...errors, result.error] : errors))(
          schema.safeParse(x)
...
export const CredentialAssertionResponse = z.object({ "id": z.string().describe("The credential's identifier."), "type": z.literal("#/$defs/PublicKeyCredentialType").describe("Specifies the credential type represented by this object."), "rawId": z.literal("#/$defs/URLEncodedBase64").describe("The credential's identifier. Since we base64-encode raw data, this is the same as `id`."), "clientExtensionResults": z.literal("#/$defs/AuthenticationExtensionsClientOutputs").describe("A map containing identifier -> client extension output entries produced by any extensions that may have been used during login.").optional(), "authenticatorAttachment": z.any().superRefine((x, ctx) => {
  ...
  }).describe("If this member is present, eligible authenticators are filtered to only authenticators attached by the specified mechanism.").optional(), "response": z.literal("#/$defs/AuthenticatorAssertionResponse").describe("The authenticator's response to the request to generate a login assertion.") }).strict().describe("The raw response returned to us from an authenticator when we request a credential for login.");
...
export const CredentialCreationResponse = z.object({ "id": z.string().describe("The credential's identifier."), "type": z.literal("#/$defs/PublicKeyCredentialType").describe("Specifies the credential type represented by this object."), "rawId": z.literal("#/$defs/URLEncodedBase64").describe("The credential's identifier. Since we base64-encode raw data, this is the same as `id`."), "clientExtensionResults": z.literal("#/$defs/AuthenticationExtensionsClientOutputs").describe("A map containing identifier -> client extension output entries produced by any extensions that may have been used during registration.").optional(), "authenticatorAttachment": z.any().superRefine((x, ctx) => {
    const schemas = [z.literal("#/$defs/PlatformAttachment"), z.literal("#/$defs/CrossPlatformAttachment")];
    const errors = schemas.reduce(
      (errors: z.ZodError[], schema) =>
        ((result) => ("error" in result ? [...errors, result.error] : errors))(
  ...
  }).describe("If this member is present, eligible authenticators are filtered to only authenticators attached by the specified mechanism.").optional(), "response": z.literal("#/$defs/AuthenticatorAttestationResponse").describe("The authenticator's response to the request to generate a registration attestation."), "transports": z.array(z.string()).describe("@deprecated Deprecated due to upstream changes to the API. Use {@link AuthenticatorAttestationResponse.transports} instead.").optional() }).strict().describe("The raw response returned to us from an authenticator when we request a credential for registration.");
export const CredentialDescriptor = z.object({ "type": z.literal("#/$defs/PublicKeyCredentialType").describe("Type of the credential to use."), "id": z.literal("#/$defs/URLEncodedBase64").describe("The ID of a credential to allow/disallow."), "transports": z.array(z.any().superRefine((x, ctx) => {
    const schemas = [z.literal("#/$defs/TransportUSB"), z.literal("#/$defs/TransportNFC"), z.literal("#/$defs/TransportBLE"), z.literal("#/$defs/TransportHybrid"), z.literal("#/$defs/TransportInternal")];
    const errors = schemas.reduce(
      (errors: z.ZodError[], schema) =>
        ((result) => ("error" in result ? [...errors, result.error] : errors))(
  ...
  })).describe("The authenticator transports that can be used.").optional() }).strict().describe("Specifies a credential for use by the browser when it calls `navigator.credentials.create()` or `navigator.credentials.get()`.");
...
export const SessionData = z.object({ "challenge": z.string().describe("Challenge that was sent to the browser."), "user_id": z.string().describe("ID of the user being registered or logged in."), "allowed_credentials": z.array(z.string()).describe("Credentials allowed in this login or registration ceremony.").optional(), "expires": z.string().datetime().describe("When this data expires"), "userVerification": z.any().superRefine((x, ctx) => {
  ...
  }).describe("Required user verification in this login or registration ceremony.").default("preferred"), "extensions": z.literal("#/$defs/AuthenticationExtensions").describe("Contains additional parameters requesting additional processing by the client and authenticator.").optional() }).strict().describe("Data that should be stored securely (anti-tamper) by the Relying Party for the duration of the registration or login ceremony.");
...
export const URLEncodedBase64 = z.string().describe("URL-encoded base64 data.");
